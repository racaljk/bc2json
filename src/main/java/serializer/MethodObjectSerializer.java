...
public class MethodObjectSerializer implements JsonSerializer<MethodObject> {
    ...
    private static JsonObject constructPureJsonMethod(CoreMethodProperties method){
    ...
    private static JsonObject constructReadableJsonMethod(CoreMethodProperties method, MethodObject m){
        JsonObject jsonMethod = new JsonObject();
        ...
        if (method.getIntArrayCode() != null) {
            StringBuilder sb = new StringBuilder();
            int[] opcodes = method.getIntArrayCode();

            for(int i=0;i<opcodes.length;i++){
                // mnemonic opcode
                sb.append(Readability.opcode2MnemonicStr(opcodes[i]));
                // operand
                switch (opcodes[i]) {
                    ...
                    case Mnemonic.ifgt: case Mnemonic.ifle: case Mnemonic.iflt: case Mnemonic.ifne: case Mnemonic.ifnonnull:
                    case Mnemonic.ifnull: case Mnemonic.instanceof$: case Mnemonic.invokespecial: case Mnemonic.invokestatic:
                    case Mnemonic.invokevirtual: case Mnemonic.jsr: case Mnemonic.ldc_w: case Mnemonic.ldc2_w: case Mnemonic.new$:
                    case Mnemonic.putfield: case Mnemonic.putstatic: case Mnemonic.sipush:
    ...
    public JsonElement serialize(MethodObject m, Type serializeType, JsonSerializationContext jsonSerializationContext) {
        ...
        for (CoreMethodProperties method : methods) {
            if (option.isMoreReadable()) {
                jsonMethods.add(constructReadableJsonMethod(method,m));
            } else {
                jsonMethods.add(constructPureJsonMethod(method));
            }
        }
        return jsonMethods;
    }
